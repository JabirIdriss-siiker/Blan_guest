...
async function createAutomaticMission(booking, apartment) {
  ...
  // Récupère le staff et l'admin (utilise le cache)
  ...
  console.log(`✅ Mission créée [${mission._id}]`);
  return mission;
}

// Fonction générique de traitement de bookings (optimisée)
async function processBookings(filter, label) {
  ...
  // 4. Batch query pour vérifier les missions existantes
  ...
  console.log(`  → ${newBookings.length} nouvelles missions à créer`);

  if (newBookings.length === 0) return [];

  // 6. Traitement en parallèle (par batch de 5 pour éviter la surcharge)
  const results = [];
  const batchSize = 5;
...
async function processUpcomingBookings() {
  const now = new Date();
  const in5Days = new Date(now.getTime() + 10*24*60*60*1000);
