...
const getDefaultMissionTemplate = (apartment, booking) => ({
  title:       `Nettoyage après départ - ${apartment.name}`,
  description: `Nettoyage automatique post-départ de ${booking.guestName || 'client'} (${booking.source})`,
  priority:    'Normale',
  cleaningPrice: apartment.cleaningPrice || 0,
  checklist: [
    'Nettoyer et désinfecter toutes les surfaces',
    'Changer les draps et serviettes',
    'Nettoyer la salle de bain complètement',
    'Nettoyer la cuisine et vérifier les équipements',
    'Passer l\'aspirateur et nettoyer les sols',
    'Vérifier et nettoyer les fenêtres',
    'Vider toutes les poubelles',
    'Vérifier l\'état général de l\'appartement',
    'Prendre des photos après nettoyage'
  ].map(task => ({ task })),
  instructions: apartment.instructions || 'Suivre la procédure standard.',
...
// Trouve le staff le moins chargé pour une date donnée (optimisé)
async function getAvailableStaff(missionDate) {
  const now = Date.now();
  
  // Utilise le cache pendant 5 minutes
  if (!staffCache || now > cacheExpiry) {
    staffCache = await User.find({ role: 'Staff de ménage', isActive: true }).lean();
  ...
  // Comptage des missions par staff sur la journée
...
// Crée une mission unique de manière atomique (SOLUTION PRINCIPALE)
async function createAutomaticMission(booking, apartment) {
  console.log(`🔨 Création mission auto pour booking ${booking._id}`);
  ...
  // Récupérer le staff et l'admin (utilise le cache)
  ...
  // Cela garantit qu'une seule mission auto sera créée par jour/appartement
  const filter = {
  ...
  try {
    ...
    // Vérifier si c'est une nouvelle mission (upserted) ou existante
    const isNewMission = result.createdAt.getTime() > (Date.now() - 5000); // Créée dans les 5 dernières secondes
    
    if (isNewMission) {
      console.log(`✅ Nouvelle mission créée [${result._id}] pour ${apartment.name} le ${missionStart.toISOString().split('T')[0]}`);
      
      // Créer la tâche de blanchisserie pour cette nouvelle mission
      try {
        await createLaundryTaskForMission(result, apartment, admin);
      } catch (laundryError) {
        console.error('⚠️ Erreur création tâche blanchisserie:', laundryError);
        // Ne pas faire échouer la création de mission si la tâche de blanchisserie échoue
      }
      ...
      setImmediate(async () => {
        try {
          ...
          console.log(`✉️ Notification envoyée à ${staff.email}`);
        } catch (err) {
          console.error('⚠️ Erreur notification:', err);
        }
      });
    } else {
      console.log(`↩️ Mission auto existe déjà pour ${apartment.name} le ${missionStart.toISOString().split('T')[0]} (ID: ${result._id})`);
    }

    return result;
  } catch (error) {
    // Gestion spéciale des erreurs de doublon (au cas où l'index unique se déclenche)
    if (error.code === 11000 || error.name === 'MongoServerError') {
      console.log(`↩️ Doublon détecté (index unique), récupération de la mission existante pour ${apartment.name} le ${missionStart.toISOString().split('T')[0]}`);
    ...
    console.error(`❌ Erreur création mission pour ${apartment.name}:`, error);
    throw error;
  }
}

// Fonction générique de traitement de bookings (optimisée et sécurisée)
async function processBookings(filter, label) {
  console.log(`🔍 Traitement des bookings (${label})…`);
  
  // 1. Récupère tous les bookings correspondants
  const bookings = await Booking.find(filter).lean();
  console.log(`  → ${bookings.length} bookings récupérés`);
  
  if (bookings.length === 0) return [];

  // 2. Batch query pour récupérer les apartments
  ...
  // Créer un map pour les apartments
  ...
  console.log(`  → ${validBookings.length} bookings avec apartments actifs`);

  if (validBookings.length === 0) return [];

  // 4. Grouper les bookings par appartement et jour pour détecter les doublons
  ...
  for (const [key, dayBookings] of bookingsByApartmentDay) {
    if (dayBookings.length > 1) {
      console.log(`  📅 ${key}: ${dayBookings.length} bookings pour la même date de mission, traitement du dernier checkout seulement`);
    }
  ...
  console.log(`  → ${bookingsToProcess.length} bookings uniques à traiter (après déduplication par jour de mission)`);

  if (bookingsToProcess.length === 0) return [];

  // 6. Traitement séquentiel pour éviter les conflits de concurrence
  // (Plus sûr que le parallélisme pour les opérations atomiques)
  const results = [];
  ...
  for (const booking of bookingsToProcess) {
    try {
      ...
      results.push({ 
        ...
        success: true,
        created: mission.createdAt.getTime() > (Date.now() - 10000) // Créée dans les 10 dernières secondes
      });
    } catch (err) {
      console.error(`✖️ Erreur booking ${booking._id}:`, err.message);
      results.push({ 
        bookingId: booking._id, 
        success: false, 
        error: err.message 
  ...
  console.log(`✅ Résultats: ${successful} nouvelles missions, ${existing} existantes, ${failed} échecs`);
  return results;
}

// Réservations finissant dans 5 jours (optimisé)
// Réservations tombant entre aujourd'hui et 30 jours (overlap inclus)
async function processUpcomingBookings() {
  ...
  return processBookings(
    ...
    'à venir (30 jours overlap)'
  );
}


// Réservations terminées il y a moins de 24h (optimisé)
